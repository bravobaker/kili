...
# 1.3 Почему количество документов меньше количества вхождений? Ответ: потому что в одном документе несколько раз может быть употреблено искомое слово
```
...
```
# 2.1 К какой части речи принадлежит форма _подыми_ в первом предложении выдачи? Запишите ее помету:Лемма	подымить (см. в словарях)
Грамматика	глаг, нп, св, повел, действ, 2л, ед, disamb
Лемма	поднять (см. в словарях)
Грамматика	глаг, св, повел, действ, 2л, ед, disamb
Семантика основная	ca:caus, d:pref, t:move
Семантика дополнительная	d:pref
Доп. признаки	adash, amark, capital, first, словарн, numred, posred 
```
...
```
# 3.2 Первое вхождение (год): 1773
# 3.3 Первое вхождение (лемма): цыган
```
...
```
# 3.4 Ответ: потому что он предназначен для поиска по словарной форме
```
